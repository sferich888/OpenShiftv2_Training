...
<body>
 <section class='container'>
          <hgroup>
            <h1>Welcome to the Clustering JBoss for OpenShift Training</h1>
          </hgroup>

        <div class="row">
          <section class='col-xs-12 col-sm-12 col-md-12'>
             <h2>OpenShift JBoss Clustering</h2>
                <p>On of the cool parts about OpenShift is its ability to wire together resources. For example, you can automaticly wire up multiple JBoss Gears into a single JBoss Cluster by simply scaling your JBoss application. To see this in action run the following command:</p>

<pre>
# rhc cartridge scale jbosseap-6 -a <%=System.getenv("OPENSHIFT_APP_NAME") %> --min 2
</pre>

                <p>What this should do is create a second gear for your application (if this training was created as a scaled application), and show you two enteries below:</p>
                <ul>
                    <%
                        ...
                        for (int i = 0; i < node_count; i++){ 
                            if (i == 1){
                                example_node = nodes[i]; 
                            }
                            out.write("<li>");
                            out.write(nodes[i].split("\\[")[0].toString()); 
                            out.write("</li>");
                        }  
                     %>
                </ul>

                <p>To see the JBoss Cluster working (sharing sessions), between JBoss servers, use the <a href="/cluster-example/ClusterServelet">cluster example</a> to see your current session and what server the session is processed by.</p>

                <p>After you have this information, use the following to disable 1 of the 2 gears.</p>
                <ul>
                    <li><b>Note:</b> A command will apear below if the instructions above are followed.</li>
                    <ul>
                        <li>Fill in the <b>SERVED_BY_UUID</b> with the data from the <a href="/cluster-example/ClusterServelet">cluster example</a></li>
                        <li>Fill in the <b>SERVER_DNS</b> with the coresponding host above. Be aware that the head gear's UUID is not listed in the hosts above.</li>
                    </ul>
                </ul>
<pre>
<% if ( node_count >= 2 ) out.write("ssh SERVED_BY_UUID@SERVER_DNS jbosseap/bin/control stop" ); %>
</pre>
                <p>Then re-visit the <a href="/cluster-example/ClusterServelet">cluster example</a> page to see that the hosted location of the servlet has changed.</p>

             <h2>Self Examination</h2>
             <p>Using the commands above set this application up so that it uses exactly 4 gears. Be sure to restart the application after doing this, for the web page to pick up the changes.</p>
             <p>Self Test: <b><% if( node_count == 4 ) out.write("PASS"); else out.write("FAIL"); %></b></p>
